...
Versions tracked by this fork (from `package.json`):

| Package | Version |
|---|---|
| React / React DOM | `^19.2.4` |
| MUI (`@mui/material`, `@mui/icons-material`) | `^7.3.8` |
| Emotion (`@emotion/react`, `@emotion/styled`, `@emotion/cache`) | `^11.14.0` |
| MobX / mobx-react | `^6.15.0` / `^9.2.1` |
| TypeScript (build-time) | `^5.9.3` |
| `@jbrowse/*` | `^4.1.14` |

These versions follow whatever `products/jbrowse-web` in [`GMOD/jbrowse-components@v4.1.14`](https://github.com/GMOD/jbrowse-components/tree/v4.1.14/products/jbrowse-web) shipped with, and will move on each `scripts/upgrade.sh` cycle. If you consume the npm package, install matching majors of React, MUI, and Emotion in your host app — mismatched Reacts break hooks, and mismatched MUI/Emotion breaks the theme context.
